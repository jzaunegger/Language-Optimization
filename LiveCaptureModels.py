...
class SentimentModel(torch.nn.Module):
    ...
    def __init__(self, vocab_size, embed_size, hidden_size, num_classes, seq_len):
        ...
        #self.soft = torch.nn.Softmax(dim=num_classes)
    ...
    def forward(self, input_data):
        ...
        #print('Logits : {}'.format(linear_out))
        #print('Sigmoid: {}'.format(sig_out))
        #print('Softmax: {}'.format(sof_out))
...
def evaluate(model, loader, device, train_type):
    model.eval()
    correct, total = 0, 0
    predictions = []
    true_labels = []
    ...
    for tokens, dir_labels, sent_labels in loader:
        ...
        for i in range(0, len(outputs)):
            ...
            if train_type == 'sent':
                actual_label = torch.argmax(sent_labels[i]).cpu().detach().numpy()
            elif train_type == 'dir':
...
def train(model, train_loader, valid_loader, epochs, device, optimizer, logger, train_type):
        epoch_labels = []
        loss_history = []
        train_acc_hist = []
        valid_acc_hist = []
        ...
        for i in range(0, epochs):
            ...
            for tokens, dir_labels, sent_labels in train_loader:
                ...
                if train_type == 'sent': sent_labels = sent_labels.to(device)
                elif train_type == 'dir': dir_labels  = dir_labels.to(device)

                optimizer.zero_grad()
                logits, probs = model(tokens)

                if train_type == 'sent':
                    loss = loss_func(logits, sent_labels).to(device)
                elif train_type == 'dir':
            ...
            train_acc, train_preds, train_true = evaluate(model, train_loader, device, train_type)
            valid_acc, valid_preds, valid_true = evaluate(model, valid_loader, device, train_type)
